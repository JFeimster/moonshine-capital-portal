    ...
    <div className="bg-neo-white min-h-screen text-neo-black">
      <div className="bg-neo-black text-neo-white py-16 px-6 md:px-12 border-b-4 border-neo-green">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-xl md:text-2xl font-medium text-neo-cream/90 max-w-3xl">Browse broker coverage by market segment so founders can jump straight to partners who already understand the operating reality.</p>
        </div>
      </div>
        ...
        {industries.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {industries.map((industry) => (
              <Link key={industry.slug} href={`/industries/${industry.slug}`} className="bg-neo-cream border-4 border-neo-black p-6 shadow-brutal hover:-translate-y-1 transition-transform">
