    ...
    <main className="min-h-screen bg-neo-cream px-6 py-10 text-neo-black md:px-10">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="border-4 border-neo-black bg-neo-white p-6 shadow-[12px_12px_0_0_rgba(0,0,0,1)]">
          ...
          <p className="mt-3 max-w-4xl text-base font-medium leading-relaxed text-neo-black/80">Read-only broker coverage surface for profile visibility, assigned tools, featured utility depth, and fallback exposure. This is the operator view for broker utility quality before CRUD or sync workflows exist.</p>
        </section>
        ...
        {loadFailed && (
          <section className="border-4 border-neo-black bg-neo-yellow p-5 shadow-[8px_8px_0_0_rgba(0,0,0,1)]">
            ...
            <p className="mt-2 font-bold text-neo-black/80">The admin broker coverage page loaded, but canonical/compatibility broker data could not be fetched. Check Notion persistence first, then any optional downstream adapter.</p>
          </section>
        )}
        ...
        <section className="grid gap-6 lg:grid-cols-[1.5fr_0.5fr]">
          <div className="overflow-hidden border-4 border-neo-black bg-neo-white shadow-[10px_10px_0_0_rgba(0,0,0,1)]">
            <div className="grid grid-cols-5 border-b-4 border-neo-black bg-neo-black px-5 py-4 text-xs font-black uppercase tracking-wide text-neo-white">
              <span>Name</span><span>Slug</span><span>Assigned</span><span>Featured</span><span>Profile</span>
            </div>
            {brokerRows.length > 0 ? (
              brokerRows.map((row) => (
                <div key={row.slug} className="grid grid-cols-5 items-center border-b-2 border-neo-black px-5 py-4 text-sm font-bold uppercase tracking-wide last:border-b-0">
                  <span>{row.name}</span><span>{row.slug}</span><span>{row.assignedTools}</span><span>{row.featuredAssignments}</span>
                  <Link href={`/${row.slug}`} className="underline">{row.fallback ? 'Fallback' : 'Live'}</Link>
                </div>
              ))
            ) : (
              <div className="border-b-2 border-neo-black px-5 py-6 text-sm font-bold uppercase tracking-wide last:border-b-0">No broker rows available. The page is guarded so deployment does not fail when upstream broker data is unavailable.</div>
            )}
          </div>

          <BrokerStatusToggle currentApprovalStatus="approved" currentProfileStatus="published" />
        </section>
      </div>
    </main>
  );
