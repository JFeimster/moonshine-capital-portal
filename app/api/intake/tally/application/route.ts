...
export async function POST(req: NextRequest) {
  try {
    ...
    // Contract: accepts pre-normalized JSON (for example from n8n), not a raw Tally webhook.
    ...
    // Deterministic identity makes retries/idempotent submissions converge on the same
    // canonical partner even before the persistence adapter is fully productionized.
