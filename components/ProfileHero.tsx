...
export function ProfileHero({ broker }: ProfileHeroProps) {
  ...
  return (
    <div className="bg-neo-black text-neo-white border-4 border-neo-black shadow-brutal mb-16 relative overflow-hidden">
      <div className="absolute top-0 right-0 w-64 h-64 bg-neo-pink transform translate-x-1/2 -translate-y-1/2 rotate-45 opacity-20"></div>

      <div className="p-8 md:p-16 flex flex-col md:flex-row gap-12 items-center relative z-10">
        <div className="w-48 h-48 md:w-64 md:h-64 bg-neo-white border-4 border-neo-black shadow-[8px_8px_0px_0px_rgba(255,255,255,0.2)] flex-shrink-0 overflow-hidden">
          {broker.profileImage ? (
            <img src={broker.profileImage} alt={broker.fullName} className="w-full h-full object-cover grayscale contrast-125" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-neo-black font-black text-6xl">
